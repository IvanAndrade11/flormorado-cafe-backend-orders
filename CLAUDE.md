...
**Formulario de contacto (`/contact`, `FMC-0019`).** A diferencia de un pedido, un mensaje de contacto no se guarda en D1 — no hay tabla para eso y no se creó una solo para este endpoint. El correo que le llega al negocio *es* el único registro, así que el orden de operaciones de `routes/orders.ts` (persistir → responder → notificar en `waitUntil`) no aplica aquí: `routes/contact.ts` espera el resultado de Resend antes de responder, y devuelve `502` si falla, para que el frontend reintente (ya lo hace en `429`/`5xx`) en vez de recibir un `201` que daría el mensaje por entregado cuando en realidad se perdió. Reutiliza `ORDERS_EMAIL_TO` — el mismo buzón que recibe el resumen de pedidos — en vez de una variable nueva, porque es el mismo buzón el que debe atender ambos. El correo del cliente va como `reply-to`, no en el cuerpo del mensaje, para que el negocio responda con un solo clic sin tener que copiarlo.
